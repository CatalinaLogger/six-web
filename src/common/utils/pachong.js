...
function startSpider(x) {
  ...
  http.get(x, function (res) {
    ...
    res.on('end', function () {
      ...
      var newsTitle = $('.item-title').text().trim()
...
function savedImg($, newsTitle) {
  $('.post-container img').each(function (index, item) {
    var imgTitle = newsTitle + index
    var imgFilename = imgTitle + '.jpg'
    var imgSrc = $(this).attr('src') // 获取图片的url
    ...
    request(imgSrc).pipe(fs.createWriteStream('./image/' + newsTitle + '---' + imgFilename)) // 通过流的方式，把图片写到本地/image目录下，并用新闻的标题和图片的标题作为图片的名称。
  })
}
